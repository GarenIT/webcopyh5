<!-- 流水记录 -->
<template>
    <div class="page_record page_audit_record">
...
const tableHeader = computed(() => {
    return [ // 表头
        { title: t('auditRecord_page_audit'), key: 'title', size: 'm' },
        { title: t('auditRecord_page_auditMon'), key: 'amount', size: 'm' },
        { title: t('auditRecord_page_auditProgress'), key: 'progess', size: 'm' },
        { title: t('auditRecord_page_state'), key: 'type', size: 'm' },
        { title: t('auditRecord_page_startTime'), key: 'create_time', size: 'm' },
    ]
})


const optionsStatus = computed(() => { // 状态
    const options = Object.keys(AuditStatusMap).map((key: string) => {
        return {
            label: AuditStatusMap[key],
            value: Number(key)
        }
    })
