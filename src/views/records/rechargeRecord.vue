<!-- 充值记录 -->
<template>
    <div class="page_record page_recharge_record">
...
import { RechagreStatusMap, CurrencyMap, RechagreStatusColorMap } from "@/enums/walletEnum"
...
const tableHeader = computed(() => {
    return [ // 表头
        ...
        { title: t('rechargeRecord_page_amount'), key: 'pay_money', size: 's' },
...
const resultHandle = (rs: any) => { // 数据处理
    ...
    result.list = result.list.concat(rs.recharge_record_list)
}
...
const rowHandle = (row: any, key: string) => { // 格子数据处理
    ...
    switch (key) {
        case "way_id":
            ...
            break
        case "order_status":
            rs = `<span style="color:${RechagreStatusColorMap[val]}">${RechagreStatusMap[val]}</span>`
            break
        case "currency":
            rs = CurrencyMap[1]
            break
