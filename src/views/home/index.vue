  ...
  <div class="home">
    ...
    <div class="top flex space_between a_i_center">
      ...
      <div class="right_box flex_center">
        <div v-if="!hasLogin" class="log_btn f_s_12 m_r_5 common_btn" @click="toLogin">{{ $t('home_page_login')
          }}
        </div>
        <div v-if="!hasLogin" class="sign_btn f_s_12 m_r_5 common_btn" @click="toReg">{{ $t('home_page_signUp')
          }}
        </div>
        <div v-if="hasLogin" class="balance m_r_10 flex_center">
          <img class="balance_coin" src="/imgs/home/currency.webp">
          <span class="p_5">{{ formatNumber(String(roleInfo.money + roleInfo?.bank_money)) }}</span>
    ...
    <div class="section">
      ...
      <div class="grid space_around m_t_4">
        <template v-for="(item, i) in state.gridList">
          <nut-popover custom-class="grid_setlector" v-model:visible="item.show" bg-color="transparent" duration="0.1"
            location="bottom" :show-arrow="false" :offset="[56, -5]">
            <template #reference>
            ...
            <template #content>
              <div class="grid_dropdown bd_r_12" v-if="item.selectList">
...
import { handleOpenLink, formatNumber } from '@/utils/publicMethods'
